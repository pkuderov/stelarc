...
def run_mnist():
    ...
    config = SimpleNamespace(
        ...
        ds=SimpleNamespace(
            ds='mnist', grayscale=True, center="pixel", lp_norm=2,
        ),
        env=SimpleNamespace(
        ...
        agent=SimpleNamespace(
            learning_rate=0.008,
            lr_min=10.0, lr_decay_slope=1.0, lr_decay_speed=1.0,

            discount_gamma=0.99, gae_lambda=0.0,
            val_loss_scale=0.25,

            ent_loss_scale=0.08, ent_loss_heads_scale=(1.0, 0.1, 0.4, 0.4),
            # ent_loss_scale_decay=0.999,
            # ent_loss_scale_max_decay=400.,
        ),

        run=SimpleNamespace(
            n_epochs=1, n_batches=10_000, n_batch_steps=4,
            # n_epochs=1, n_batches=20, n_batch_steps=2,
            eval_configs=[
